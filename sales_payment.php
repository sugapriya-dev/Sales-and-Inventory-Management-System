...
$outstanding = 0;
if ($customer_id > 0) {
    $outstanding = cust_outstanding($customer_id);
}
...
if (isset($_POST['makePayment'])) {
    ...
    // Get customer name
    $sup_result = $conn->query("SELECT customername FROM customers WHERE id = $customer_id");
    $sup_data = $sup_result->fetch_assoc();
    $customername = $sup_data['customername'];
    ...
    try {
        // 1. Insert into sales_payments table first
        $particulars = "Payment to Customer - " . $customername;
        if (!empty($remarks)) {
            $particulars .= " - " . $remarks;
        }
        
        $payment_insert = $conn->prepare("INSERT INTO sales_payments 
            (customer_id, payment_date, amount, payment_mode, bank_name, reference_no, particulars) 
            VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        if (!$payment_insert->execute()) {
            throw new Exception("Error inserting into sales_payments: " . $conn->error);
        }
        
        // Get the generated customer_id
        $customer_payment_id = $conn->insert_id;
        
        if ($payment_mode == "cash") {
            // Insert into cash_in_hand
            $particulars = "Payment to Customer - " . $customername;
            $amt = $amount; // Postive for payment 
            ...
            $insert->bind_param("sisssd", 
                $payment_date,
                $customer_payment_id,  // Using customer_payment_id as reference_id(sales_payment table id)
                $reference_no,
                $customer_id,
                $particulars,
                $amt
    ...
    echo "<script>alert('$message'); window.location='cust_ledger.php?customer_id=$customer_id';</script>";
}
?>
...
<html>
<head>
    <title>Make Payment - Sales</title>
    <style>
        body {
...
<body>
    ...
    <div class="container">
        <h2>Make Payment to Customer</h2>
        ...
        <div class="info-box">
            <div class="info-row">
                ...
                <span class="info-value"><?php echo $customer['customername'] ?? ''; ?></span>
            </div>
            <div class="info-row">
                <span class="info-label">Phone:</span>
                <span class="info-value"><?php echo $customer['phoneno'] ?? ''; ?></span>
            </div>
            <div class="info-row">
        ...
        <form method="POST">
            ...
            <button type="button" class="cancel-btn" onclick="window.location='cust_ledger.php?customer_id=<?php echo $customer_id; ?>'">Cancel</button>
        </form>
    </div>
</body>
</html>