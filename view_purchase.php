...
// Get filter values
$supplier_id = isset($_GET['supplier_id']) && $_GET['supplier_id'] != '' ? intval($_GET['supplier_id']) : 0;
...
if($from_date != '') {
    $query .= " AND p.date >= '$from_date'";
    $countQuery .= " AND p.date >= '$from_date'";
}
if($to_date != '') {
    $query .= " AND p.date <= '$to_date'";
    $countQuery .= " AND p.date <= '$to_date'";
}
if($invoice_no != '') {
    $query .= " AND p.invoiceno LIKE '%$invoice_no%'";
    $countQuery .= " AND p.invoiceno LIKE '%$invoice_no%'";
}
...
<div class="container">
    ...
    <div class="filter-section">
        <div class="filter-group">
            ...
            <select name="supplier_id">
                ...
                <?php
                $supplier = $conn->query("SELECT id, suppliers_name FROM suppliers ORDER BY suppliers_name");
                while($sup = $supplier->fetch_assoc()):
                    $selected = (isset($_GET['supplier_id']) && $_GET['supplier_id'] == $sup['id']) ? 'selected' : '';
                ?>
        ...
        <div class="filter-group">
         ...
         <div class="filter-group">
            ...
            <a href="view_purchase.php" class="reset-btn">Reset</a>
        </div>
    </div>
</form>
  ...
  <div class="top-links">
           ...
           <?php while($row =$res->fetch_assoc()) :?>
            <tr>
                   ...
                   <td><?php echo htmlspecialchars($row['grand_total']); ?></td>
                    <td>
                        <a href="edit_purchase1.php?id=<?php echo $row['id'];?>">Edit</a>
                         <a href="delete_purchase.php?id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure?');">Delete</a>
                    </td>
      ...
      <div class="pagination">
            <?php if($page > 1): ?>
                <a href="?page=<?php echo $page-1; ?>&supplier_id=<?php echo $supplier_id; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&invoice_no=<?php echo $invoice_no; ?>">« Prev</a>
            <?php else: ?>
            ...
            <?php for($i = 1; $i <= $totalPages; $i++): ?>
                ...
                <?php else: ?>
                    <a href="?page=<?php echo $i; ?>&supplier_id=<?php echo $supplier_id; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&invoice_no=<?php echo $invoice_no; ?>">
                        <?php echo $i; ?>
                    </a>
                <?php endif; ?>
            <?php endfor; ?>
            ...
            <?php if($page < $totalPages): ?>
                <a href="?page=<?php echo $page+1; ?>&supplier_id=<?php echo $supplier_id; ?>&from_date=<?php echo $from_date; ?>&to_date=<?php echo $to_date; ?>&invoice_no=<?php echo $invoice_no; ?>">Next »</a>
            <?php else: ?>
